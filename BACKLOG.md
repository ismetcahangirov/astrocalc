...
- Natal chart detail rows are now expandable accordions — mobile, backend,
  ...
  section taps open to reveal what that placement means, with only one row open
  at a time (opening one closes the previous). The meaning text already came
  from `/interpretations/for-chart` for planet-sign, planet-house and aspect;
  the old flat "Your Reading" / "Aspects" lists were folded into the accordions
  so nothing is shown twice. Angle rows (Asc/MC) stay plain — no interpretation
  content for them in scope. Houses had **no** interpretation text, so a new
  `house` interpretation category (12 generic whole-house meanings × 4 locales,
  category `house`) was added to the existing original-content generator — no
  external API, keeping content original and offline-consistent. Wiring:
  calc-engine `houseSubjectKey()` + enumeration (1,332 → 1,344 subjects);
  `seedContent.ts` `composeHouse()` reusing the existing `HOUSE_AREA`/
  `HOUSE_ORDINAL` banks (5,328 → 5,376 seed rows); `getForComputedChart` returns
  `houses[]`; route `categorySchema` accepts `house`; mobile `formatChartDetails`
  attaches each row's interpretation `subjectKey`, and `NatalChartScreen` renders
  rows via a new single-open `AccordionRow`. calc-engine + backend + mobile (213)
  tests green; tsc clean. **Deploy note:** the 48 new `house` rows must be
  written by re-running `db:seed:interpretations` (idempotent — only inserts
  missing rows) so house accordions show text. Pre-existing repo `expo lint`
  config error (`no-unassigned-vars` rule missing) is unrelated to this work.
